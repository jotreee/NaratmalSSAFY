...
const PostGwangjuItem = ({ idx, postData }) => {
    ...
    useEffect(() => {
      const fontDataDiv = document.getElementById(`gwangju_${idx}`);
      const fontDataTextArea = document.getElementById(
        `gwangju_textarea_${idx}`
      );
      const postColor = document.getElementById(`gwangju_color_${idx}`)
    ...
    .then(res => {
      swal({
        ...
        window.location.href='/post/gwangju'
      });
    })
    .catch(err => {
      swal({
    ...
    return (
      <div className="fontData" id={`gwangju_${idx}`}>
      <link
        rel="stylesheet"
        type="text/css"
      ...
      />
      <div
        className="info_box" id={`gwangju_color_${idx}`}
      >
...
export default PostGwangjuItem;
